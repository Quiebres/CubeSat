---
title: "Análisis de Sentimientos Cubesat"
author: "Odalis Reyes e Ivan Maldonado"
date: "10/03/2020"
output:
...
```{r}
search.string <- "#Quetzal1"
result.term <- searchTwitter(search.string, lang = "es", n = 1000)
head(result.term)
...
findAssocs(dtm, 'uvg', 0.2)
```

```{r}

findAssocs(dtm, 'satelite', 0.2)
...
ggplot(data=Sentimentscores,aes(x=Sentimiento,y=Puntaje))+geom_bar(aes(fill=Sentimiento),stat = "identity")+
  ...
  xlab("Sentimiento")+ylab("Puntaje")+ggtitle("Sentimientos de las personas con el hashtag #Quetzal1")
```
